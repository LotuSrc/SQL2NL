...
class Graph2SeqNN(object):
    ...
    def optimized_gcn_encode(self):
        ...
        for layer in range(self.sample_layer_size):
            ...
            if layer > 15:
                fw_aggregator = self.fw_aggregators[15]
            else:
            ...
            if self.graph_encode_direction == "bi":
                if layer > 15:
                    bw_aggregator = self.bw_aggregators[15]
                else:
    ...
    def decode(self, encoder_outputs, encoder_state, source_sequence_length):
        with tf.variable_scope("Decoder") as scope:
            ...
            ori_batch_size = tf.shape(encoder_outputs)[0]
            ...
            if decoder_type == "greedy":
                ...
                decoder_infer = seq2seq.BeamSearchDecoder(cell=self.decoder_cell, embedding=self.word_embeddings,
                                                          start_tokens=tf.ones([ori_batch_size], dtype=tf.int32),
                                                          end_token=self.EOS,
            ...
            if decoder_type == "beam":
                ...
                self.sample_id = decoder_outputs_infer.predicted_ids[:, :, 0]
    ...
    def act(self, sess, mode, dict, if_pred_on_dev):
        ...
        fw_adj_info = np.array(batch_graph['g_fw_adj'])
        bw_adj_info = np.array(batch_graph['g_bw_adj'])
        feature_info = np.array(batch_graph['g_ids_features'])
        batch_nodes = np.array(batch_graph['g_nodes'])
