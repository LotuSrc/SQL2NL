...
#                        print(len(sample_id))
                    ...
                    acc = evaluate(type="bleu", golds=texts_dev, preds=pred_texts)
                    if_save_model = False
                    ...
                    print('Epoch', t)
                    print('Bleu on Dev: {}'.format(acc))
                    print('Best bleu on Dev: {}'.format(best_acc_on_dev))
                    ...
                    if if_save_model:
                        ...
                        path = saver.save(sess, save_path + 'model_best.ckpt', global_step=0)
                        print("Already saved model to {}".format(path))
                    saver.save(sess, '../saved_model/model_checkpoint.ckpt', global_step=t)
        ...
        with tf.Graph().as_default():
            with tf.Session() as sess:
                ...
                model_path_name = "../saved_model/model_best.ckpt-0"
                ...
                acc = evaluate(type="bleu", golds=texts_test, preds=pred_texts)
                print("bleu on test set is {}".format(acc))
